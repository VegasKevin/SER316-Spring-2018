...
public class ProjectExporter {
    ...
    public static void export(IProject prj, File f, String charset, boolean xhtml, boolean chunked, boolean navigation,
    ...
    private static void generateToc(Writer w, Vector notes) {
        ...
        if (!_chunked)
            ...
            link = id + ".html";
        write(w, "<li><a href=\"" + link + "\">" + note.getDate().getMediumDateString() + " " + note.getTitle()
                + "</a></li>\n");
        }
        write(w, "</ul></div>\n");
    }

    private static String getNoteHTML(INote note) {
        String text = "";
        StringWriter sw = new StringWriter();
        AltHTMLWriter writer = new AltHTMLWriter(sw, (HTMLDocument) CurrentStorage.get().openNote(note), _charset,
                _num);
        try {
        writer.write();
        sw.flush();
         ...
         * java.util.regex.Pattern.CASE_INSENSITIVE); Matcher m = p.matcher(text); for
         * (int i = 1; i < m.groupCount(); i++) { String g = m.group(i); String url =
         * g.split("\"")[1]; }
         */
    ...
    private static String generateNav(INote prev, INote next) {
        ...
        if (prev != null)
            s += "<div class=\"navitem\"><a href=\"" + prev.getId() + ".html\">" + Local.getString("Previous")
                    + "</a><br></br>" + prev.getDate().getMediumDateString() + " " + prev.getTitle() + "</div>";

        else
            s += "&nbsp;";
        s += "</td><td width=\"34%\" align=\"center\"><a href=\"" + output.getName()
                + "\">Up</a></td><td width=\"33%\" align=\"right\">";
        if (next != null)
            s += "<div class=\"navitem\"><a href=\"" + next.getId() + ".html\">" + Local.getString("Next")
                    + "</a><br></br>" + next.getDate().getMediumDateString() + " " + next.getTitle() + "</div>";
