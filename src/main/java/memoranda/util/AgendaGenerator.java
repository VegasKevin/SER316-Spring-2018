...
public class AgendaGenerator {
    ...
    static String generateTasksInfo(IProject p, CalendarDate date, Collection expandedTasks) {
        ...
        // TaskSorter.sort(tasks, date, TaskSorter.BY_IMP_RATE); // TODO: configurable
        // method
        ...
        // if (!((t.getStatus() == Task.ACTIVE) || (t.getStatus() == Task.DEADLINE) ||
     ...
     * @param expandedTasks
     */
    private static String expandRecursively(IProject p, CalendarDate date, ITaskList tl, ITask t, Collection expandedTasks,
            int level) {
        ...
        // if(Context.get("SHOW_ACTIVE_TASKS_ONLY").equals(new Boolean(true))) {
        // if (!((subTask.getStatus() == Task.ACTIVE) || (subTask.getStatus() ==
        // Task.DEADLINE) || (subTask.getStatus() == Task.FAILED))) {
        // continue;
        // }
        // }
     ...
     * @return
     */
    private static String renderTask(IProject p, CalendarDate date, ITaskList tl, ITask t, int level,
         ...
         * Local.getString("Should be done today") + ".</b></font></p>"; else { Calendar
         * endDateCal = t.getEndDate().getCalendar(); Calendar dateCal =
         * date.getCalendar(); int numOfDays = (endDateCal.get(Calendar.YEAR)*365 +
         * endDateCal.get(Calendar.DAY_OF_YEAR)) - (dateCal.get(Calendar.YEAR)*365 +
         * dateCal.get(Calendar.DAY_OF_YEAR)); String days = ""; if (numOfDays > 1) days
         * = Local.getString("in")+" "+numOfDays+" "+Local.getString("day(s)"); else
         * days = Local.getString("tomorrow"); s += "<p>" + Local.getString("Deadline")
        ...
        s += "<p>" + Local.getString("Deadline") + ": <i>" + t.getEndDate().getMediumDateString() + "</i> (" + days
                + ")</p>";
    ...
    static String generateEventsInfo(CalendarDate date) {
        ...
        String iurl = main.java.memoranda.ui.AppFrame.class.getResource("/ui/agenda/spacer.gif").toExternalForm();
        ...
        iurl = main.java.memoranda.ui.AppFrame.class.getResource("/ui/agenda/arrow.gif").toExternalForm();
        }
        }
    ...
    static String generateStickers(CalendarDate date) {
        String iurl = main.java.memoranda.ui.AppFrame.class.getResource("/ui/agenda/addsticker.gif").toExternalForm();
        String iurl2 = main.java.memoranda.ui.AppFrame.class.getResource("/ui/agenda/removesticker.gif")
                .toExternalForm();
        ...
        s += "\n<table border=\"0\" cellpadding=\"0\" width=\"100%\"><table width=\"100%\"><tr bgcolor=\"#E0E0E0\"><td><a href=\"memoranda:editsticker#"
                + id + "\">" + Local.getString("EDIT") + "</a></td><td width=\"70%\"><a href=\"memoranda:expandsticker#"
                + id + "\">" + Local.getString("OPEN IN A NEW WINDOW") + "</></td><td align=\"right\">" + "&nbsp;" + // without
                                                                                                                     // this
                                                                                                                     // removesticker
                                                                                                                     // link
                                                                                                                     // takes
    ...
    /*
     * we do not need this. Tasks are sorted using the Comparable interface public
     * static class TaskSorter {
     * 
     * static final int BY_IMP_RATE = 0; static final int BY_END_DATE = 1; static
     * final int BY_PRIORITY = 2; static final int BY_COMPLETION = 3;
     * 
     * private static Vector tasks = null; private static CalendarDate date = null;
     * private static int mode = 0;
     * 
     * public static long calcTaskRate(Task t, CalendarDate d) { /* A "Task rate" is
     * an informal index of importance of the task considering priority, number of
     * days to deadline and current progress.
     * 
     * rate = (100-progress) / (numOfDays+1) * (priority+1) / Calendar endDateCal =
     * t.getEndDate().getCalendar(); Calendar dateCal = d.getCalendar(); int
     * numOfDays = (endDateCal.get(Calendar.YEAR)*365 +
     * endDateCal.get(Calendar.DAY_OF_YEAR)) - (dateCal.get(Calendar.YEAR)*365 +
     * dateCal.get(Calendar.DAY_OF_YEAR)); if (numOfDays < 0) return -1; //Something
     * wrong ? return (100-t.getProgress()) / (numOfDays+1) * (t.getPriority()+1); }
     * 
     * static long getRate(Object task) { Task t = (Task)task; switch (mode) { case
     * BY_IMP_RATE: return -1*calcTaskRate(t, date); case BY_END_DATE: return
     * t.getEndDate().getDate().getTime(); case BY_PRIORITY: return
     * 5-t.getPriority(); case BY_COMPLETION: return 100-t.getProgress(); } return
     * -1; }
     * 
     * private static void doSort(int L, int R) { // Hoar's QuickSort int i = L; int
     * j = R; long x = getRate(tasks.get((L + R) / 2)); Object w = null; do { while
     * (getRate(tasks.get(i)) < x) i++; while (x < getRate(tasks.get(j)) && j > 0)
     * if (j > 0) j--; if (i <= j) { w = tasks.get(i); tasks.set(i, tasks.get(j));
     * tasks.set(j, w); i++; j--; } } while (i <= j); if (L < j) doSort(L, j); if (i
     * < R) doSort(i, R); }
     * 
     * public static void sort(Vector theTasks, CalendarDate theDate, int theMode) {
     * if (theTasks == null) return; if (theTasks.size() <= 1) return; tasks =
     * theTasks; date = theDate; mode = theMode; doSort(0, tasks.size() - 1); }
     * 
     * }
     */
}
